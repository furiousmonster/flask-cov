...
def get_tencent_data():
    ...
    url_now = 'https://view.inews.qq.com/g2/getOnsInfo?name=disease_h5'
    headers = {
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36',
        'Referer':' https://news.qq.com/zt2020/page/feiyan.htm'  # 上一个链接，伪装
    }
    r_history = requests.get(url_history, headers)
    r_now = requests.get(url_now, headers)
    res_history = json.loads(r_history.text)  # json字符串转字典
    res_now = json.loads(r_now.text)
    data_history = json.loads(res_history['data'])
    data_now = json.loads(res_now['data'])
    ...
    details = []  # 当日详细数据
    update_time = data_now["lastUpdateTime"]
    data_country = data_now["areaTree"]  # list 25个国家
...
def get_conn():
    ...
    """
    # 创建连接
    conn = pymysql.connect(host="118.195.163.65",
                           user="root",
                           password="Hlz..1997",
                           db="cov",
                           charset="utf8")
...
def get_baidu_hot():
    ...
    browser.get(url)
    # 找到展开按钮
    dl = browser.find_element_by_xpath('//*[@id="ptab-1"]/div[3]/div[11]/span')
    dl.click()
    time.sleep(1)
    # 找到热搜标签
